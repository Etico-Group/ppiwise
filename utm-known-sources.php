<?php 
if($_SERVER['HTTP_HOST']=='demo.ebizindia.com'){
	$utm_known_sources = [
		'' => [
			'Apply' => 'not-set',
			'Apply1' => 'not-set',
			'Pack_Request' => 'not-set',
			'Pack_Request1' => 'not-set',
			'Call_Back' => 'not-set',
			'Call_Back1' => 'not-set'
			],
		'ob' => [
			'Apply' => 'Outbrain - PPI Wise',
			'Apply1' => 'Outbrain - PPI Wise',
			'Pack_Request' => 'Outbrain - PPI Wise Postal',
			'Pack_Request1' => 'Outbrain - PPI Wise Postal',
			'Call_Back' => 'Outbrain - PPI Wise Call Back',
			'Call_Back1' => 'Outbrain - PPI Wise Call Back'
			],
		'outbrain' => [
			'Apply' => 'Outbrain - PPI Wise',
			'Apply1' => 'Outbrain - PPI Wise',
			'Pack_Request' => 'Outbrain - PPI Wise Postal',
			'Pack_Request1' => 'Outbrain - PPI Wise Postal',
			'Call_Back' => 'Outbrain - PPI Wise Call Back',
			'Call_Back1' => 'Outbrain - PPI Wise Call Back'
			],
		'taboola' => [
			'Apply' => 'Taboola - PPI Wise',
			'Apply1' => 'Taboola - PPI Wise',
			'Pack_Request' => 'Taboola - PPI Wise Postal',
			'Pack_Request1' => 'Taboola - PPI Wise Postal',
			'Call_Back' => 'Taboola - PPI Wise Call Back',
			'Call_Back1' => 'Taboola - PPI Wise Call Back'
			],
		'revcontent' => [
			'Apply' => 'Taboola - PPI Wise',
			'Apply1' => 'Taboola - PPI Wise',
			'Pack_Request' => 'Taboola - PPI Wise Postal',
			'Pack_Request1' => 'Taboola - PPI Wise Postal',
			'Call_Back' => 'Taboola - PPI Wise Call Back',
			'Call_Back1' => 'Taboola - PPI Wise Call Back'
		]
	];
}else{
	$utm_known_sources = [
		'' => [
			'Apply' => 'not-set',
			'Apply1' => 'not-set',
			'Pack_Request' => 'not-set',
			'Pack_Request1' => 'not-set',
			'Call_Back' => 'not-set',
			'Call_Back1' => 'not-set'
			],
		'ob' => [
			'Apply' => 'Outbrain - PPI Wise',
			'Apply1' => 'Outbrain - PPI Wise',
			'Pack_Request' => 'Outbrain - PPI Wise Postal',
			'Pack_Request1' => 'Outbrain - PPI Wise Postal',
			'Call_Back' => 'Outbrain - PPI Wise Call Back',
			'Call_Back1' => 'Outbrain - PPI Wise Call Back'
			],
		'outbrain' => [
			'Apply' => 'Outbrain - PPI Wise',
			'Apply1' => 'Outbrain - PPI Wise',
			'Pack_Request' => 'Outbrain - PPI Wise Postal',
			'Pack_Request1' => 'Outbrain - PPI Wise Postal',
			'Call_Back' => 'Outbrain - PPI Wise Call Back',
			'Call_Back1' => 'Outbrain - PPI Wise Call Back'
			],
		'taboola' => [
			'Apply' => 'Taboola - PPI Wise',
			'Apply1' => 'Taboola - PPI Wise',
			'Pack_Request' => 'Taboola - PPI Wise Postal',
			'Pack_Request1' => 'Taboola - PPI Wise Postal',
			'Call_Back' => 'Taboola - PPI Wise Call Back',
			'Call_Back1' => 'Taboola - PPI Wise Call Back'
			],
		'revcontent' => [
			'Apply' => 'RevContent - PPI Wise',
			'Apply1' => 'RevContent - PPI Wise',
			'Pack_Request' => 'RevContent - PPI Wise Postal',
			'Pack_Request1' => 'RevContent - PPI Wise Postal',
			'Call_Back' => 'RevContent - PPI Wise Call Back',
			'Call_Back1' => 'RevContent - PPI Wise Call Back'
		]
	];
}